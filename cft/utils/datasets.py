...
def img2label_paths(img_paths):
    ...
    for generation in range(1,5):
        ...
        if lookForLabelpath.exists():
            lookForLabelpath = lookForLabelpath / x.parent.parent.stem
            break
...
class LoadMultiModalImagesAndLabels(Dataset):  # for training/testing
    ...
    def __init__(self, path_rgb, path_ir, img_size=640, batch_size=16, augment=False, hyp=None, rect=False, image_weights=False,
        ...
        try:
            ...
            # self.img_files = sorted([x for x in f if x.suffix[1:].lower() in img_formats])  # pathlib
            assert self.img_files_rgb, (f'{prefix}No RGB images found', f'{prefix}No images found')
            assert self.img_files_ir, (f'{prefix}No IR images found', f'{prefix}No images found')
        except Exception as e:
        ...
        # print(self.label_files)
        cache_rgb_path = (p_rgb if p_rgb.is_file() else Path(self.label_files_rgb[0]).parent).with_suffix('.rgb.cache')  # cached labels
        try:
            cache_rgb, exists_rgb = np.load(cache_rgb_path, allow_pickle=True).item(), True  # load dict
            # print("Has key:hash", "hash" in cache_rgb)
            ...
            cache_rgb, exists_rgb = self.cache_labels(self.img_files_rgb, self.label_files_rgb, cache_rgb_path, prefix), False
            
            
        # if cache_rgb_path.is_file():
        #     print(cache_rgb_path)
        #     cache_rgb, exists_rgb = np.load((cache_rgb_path), allow_pickle=True), True  # load
        ...
        # print(self.label_files)
        cache_ir_path = (p_ir if p_ir.is_file() else Path(self.label_files_ir[0]).parent).with_suffix('.ir.cache')  # cached labels
        try:
            ...
            cache_ir, exists_ir = self.cache_labels(self.img_files_ir, self.label_files_ir, cache_ir_path, prefix), False
