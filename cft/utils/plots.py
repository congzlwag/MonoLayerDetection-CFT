...
def plot_images(images, targets, paths=None, fname='images.jpg', names=None, max_size=640, max_subplots=16):
    ...
    for i, img in enumerate(images):
        ...
        im_fuse = img[:,:,:3]
        if img.shape[-1] >= 4:
            im_fuse = (0.5*im_fuse + 0.5 * img[:,:,3:].mean(axis=2,keepdims=True))
            im_fuse = im_fuse.astype(img.dtype)
        mosaic[block_y:block_y + h, block_x:block_x + w, :] = im_fuse
        if len(targets) > 0:
...
def plot_csv_results(file='path/to/results.csv', dir=''):
    ...
    for fi, f in enumerate(files):
        ...
        # print(x.values)
        # print(data.columns)
        for i, j in enumerate(['train/box_loss', 'train/obj_loss', 'train/cls_loss', 
