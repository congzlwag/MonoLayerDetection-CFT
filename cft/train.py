...
from utils.plots import plot_images, plot_labels, plot_results, plot_evolution, plot_csv_results
...
def train_rgb_ir(hyp, opt, device, tb_writer=None):
    ...
    results_filekeys = ['epoch', 'gpu_mem', 'train/box_loss', 'train/obj_loss', 'train/cls_loss',  'train/total_loss', 'labels', 'img_size',
                     'metrics/precision', 'metrics/recall', 'metrics/mAP_0.5', 'metrics/mAP_0.75', 'metrics/mAP_0.5:0.95',
                    'val/box_loss', 'val/obj_loss', 'val/cls_loss']
    with open(results_file, 'a') as fp:
        print(','.join(results_filekeys), file=fp)
    ...
    if pretrained:
        ...
        state_dict = intersect_dicts(state_dict, model.state_dict(), exclude=exclude)  # intersect
        model.load_state_dict(state_dict, strict=False)  # load
        # LOGGER.info('Transferred all items from %s, with attempt_load' % (weights)) 
        LOGGER.info('Transferred %g/%g items from %s' % (len(state_dict), len(model.state_dict()), weights))  # report
    else:
        model = Model(opt.cfg, ch=3, nc=nc, anchors=hyp.get('anchors')).to(device)  # create
    with torch_distributed_zero_first(rank):
    ...
    train_path_ir = data_dict['train_pl']
    test_path_ir = data_dict['val_pl']
    ...
    if pretrained:
        ...
        # Results
        if ckpt.get('training_results') is not None:
            tr_csv = ','.join(list(filter(None,ckpt['training_results'].split(' '))))
            results_file.write_text(tr_csv)  # write results.txt
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        if rank in [-1, 0]:
            ...
            # Write
            with open(results_file, 'a') as f:
                s_csv = ','.join(list(filter(None,s.split(' ')))[1:])
                s_csv = str(epoch)+", "+s_csv
                f.write(s_csv +', %10.4g' * 8 % results + '\n')  # append metrics, val_loss
            if len(opt.name) and opt.bucket:
            ...
            if (not opt.nosave) or (final_epoch and not opt.evolve):  # if save
                ...
                if (epoch > 0) and (opt.save_period > 0) and (epoch % opt.save_period == 0):
                    torch.save(ckpt,  f'{opt.save_dir}/weights/epoch{epoch}.pt')
                if wandb_logger.wandb:
                    if ((epoch + 1) % opt.save_period == 0 and not final_epoch) and opt.save_period != -1:
                        wandb_logger.log_model(
                            last.parent, opt, epoch, fi, best_model=best_fitness == fi)
    ...
    if rank in [-1, 0]:
        # Plots
        if plots:
            plot_csv_results(file=results_file)  # save as results.png
            if wandb_logger.wandb:
                files = ['results.png', 'confusion_matrix.png', *[f'{x}_curve.png' for x in ('F1', 'PR', 'P', 'R')]]
                wandb_logger.log({"Results": [wandb_logger.wandb.Image(str(save_dir / f), caption=f) for f in files
                                              if (save_dir / f).exists()]})
