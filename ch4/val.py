...
def run(data,
    ...
    # Dataloader
    nchannel = data.get("nchannel", 3)
    if not training:
        if device.type != 'cpu':
            model(torch.zeros(1, nchannel, imgsz, imgsz).to(device).type_as(next(model.parameters())))  # run once
        pad = 0.0 if task == 'speed' else 0.5
        task = task if task in ('train', 'val', 'test') else 'val'  # path to train/val/test images
        dataloader = create_dataloader(data[task], imgsz, batch_size, gs, single_cls, pad=pad, rect=True,
                                       prefix=colorstr(f'{task}: '), label_path=f"{data['label']}/{task}")[0]
